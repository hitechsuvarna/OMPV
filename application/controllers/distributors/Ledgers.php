...
########## PRODUCTS ################
	public function index() {
		...
		if(isset($sess_data['user_details'][0])) {
			...
			$this->load->view('distributors/ledgers/ledgers', $data);
		} else {
	...
	public function update_product($pid) {
		...
		if(isset($sess_data['user_details'][0])) {
			...
			$pid = $this->product_model->update_product($oid, $pid, $this->input->post('product'), $this->input->post('description'), $this->input->post("hsn"), $this->input->post("unit"), $this->input->post('tax'), $this->input->post('limit'), $this->input->post('category'), $this->input->post('alias'));
			echo $pid;
		} else {
			redirect(base_url().'distributors/Account/login');
		}
...
########## PRODUCT PRICING ################
	...
	public function save_product_pricing() {
		...
		if(isset($sess_data['user_details'][0])) {
			$oid = $sess_data['user_details'][0]->iu_owner;
			$dt = date('Y-m-d H:m:s');
            $p = $this->product_model->filter_products($oid, $this->input->post('ct_id'));
            for ($i=0; $i<count($p); $i++) {
                $pid = $p[$i]->ip_id;
                $this->product_model->update_product_pricing($oid, $dt, $pid, $this->input->post('d_id'), $this->input->post('price'));
            }
			echo true;
		} else {
			redirect(base_url().'distributors/Account/login');
