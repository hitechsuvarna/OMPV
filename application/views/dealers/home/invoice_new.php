...
<script>
	$(document).ready(function() {
		...
		$('.purchase_table').on('click', '.or_nu', function(e) {
			e.preventDefault();
            window.location = "<?php echo base_url().$type.'/Home/invoice_details_new/'; ?>" + $(this).prop('id');
		});
	...
	function load_orders(from=null, to=null) {
	    $.post('<?php echo base_url().$type."/Home/fetch_invoice_list"; ?>', {
	        'f' : from, 't' : to
	    }, function(d,s,x) {
	        var a=JSON.parse(d), b="";
	        for(var i=0;i<a.orders.length;i++) {
	            b+='<tr class="or_nu" id="' + a.orders[i].id + '"> <td> <b class="order_number">#' + a.orders[i].txnno + '</b></td><td>' + a.orders[i].txndt + '</td><td class="amount"> Rs.' + a.orders[i].amount + '/-</td></tr>';
	        }
