				...
				<div class="mdl-card__supporting-text">
					...
					<?php 
					    if($section == "Distributor") {
    						...
    						echo '"> <label class="mdl-textfield__label" for="c_ifsc">IFSC</label> </div><div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label"> <p style="margin: 10px;text-align: left;">Upload Company Logo</p><br> <input type="file" name="attach_file" class="upload"> <img src="'.base_url().'assets/uploads/'.$oid.'/logo/'.$logo.'" id="p_image" style="width: 100%;"></div>';
    					}
						if(isset($user_info)) { 
							echo '<a href="';
							echo base_url().$type.'/Account/reset_password/'.$oid.'/'.$rid.'/r';
...
<script>
	$(document).ready(function() {
		$('#submit').click(function(e) {
			...
			$.post('<?php echo base_url().$type."/Account/update_details/".$oid."/".$rid; ?>', {'name' : name, 'company' : company, 'email' : email, 'phone' : phone, 'address' : address, 'gst' : gst, 'bank' : $('#c_bank').val(), 'branch' : $('#c_branch').val(), 'acc' : $('#c_acc').val(), 'ifsc' : $('#c_ifsc').val() }, function(data, status, xhr) { redirect(); }, 'text');
		});

	});
	
	function redirect() {
		window.location = '<?php echo base_url().$type."/Account/index/".$oid; ?>';
	}
</script>
</html>