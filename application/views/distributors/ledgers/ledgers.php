...
<main class="mdl-layout__content">
    <div class="mdl-grid" style="display:non;">
		...
		<div class="mdl-cell mdl-cell--12-col" style="border:1px solid #ccc; border-radius:10px; box-shadow: 0px 2px 10px #ccc; display:none;">
		    <div class="mdl-grid">
		        <div class="mdl-cell mdl-cell--4-col">
		            <div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label">
...
<script>
    $(document).ready(function() {
	    $('#update_pay').click(function(e) {
	        ...
	        var url = "";
	        if(edit_flag == true) {
	            url = '<?php echo base_url().$type."/Transactions/update_payment_txn/"; ?>' + $('#contact').val() + '/' + edit_index;
	        } else {
	            url = '<?php echo base_url().$type."/Transactions/update_payment_txn/"; ?>' + $('#contact').val() + '/';
	        }
	        var sel_txns_amt = [];
        ...
        $('#download').click(function(e) {
		    ...
		    $('.purchase_table').tableExport({
                ...
                filename: $('#contact').val() + '- Ledger - ' + $('#txn_type').val(),                         
	...
	function get_transactions(contact, type, from, to) {
	    $.post('<?php echo base_url().$type."/Transactions/get_payment_contact_txns/"; ?>', {
	        ...
	        for(var i=0;i<b.length; i++) {
	            ...
	            x+='<tr><td>' + b[i].date + '</td><td>' + b[i].txnno + '</td><td>' + b[i].type + '</td><td style="text-align:right;">'
	            if(b[i].credit != 0) {
	                x+=parseFloat(Math.round(b[i].credit * 100) / 100).toFixed(2);
	            }
	            x+='</td><td style="text-align:right;">';
	            if(b[i].debit !=0) {
	                x+=parseFloat(Math.round(b[i].debit * 100) / 100).toFixed(2);
	            }
	            x+='</td><td>';
	            if(b[i].info == "payment") {
	                x+='<button class="mdl-button mdl-js--button mdl-button--colored show_records" id="' + b[i].id + '"><i class="material-icons">launch</i></button>';
	            } else {
	               ...
	               // }
	               // x+='" type="text" id="' + b[i].id + '" class=" txn_pay_amt" placeholder="Payment Amount" value="' + b[i].pay_amt + '">';
	               x+='</div>';
	            }
	            x+='</td></tr>';
	        }
	        ...
	        var y="";
	        y+='<tr><td colspan="3">Total</td><td style="text-align:right;">' + parseFloat(Math.round(credit* 100) / 100).toFixed(2) + ' Cr</td><td style="text-align:right;">' + parseFloat(Math.round(debit* 100) / 100).toFixed(2) + ' Dr</td><td></td></tr>';
	        y+='<tr><td colspan="4">Balance</td><td style="text-align:right;">';
	        if((credit-debit) > 0) {
	            m=parseFloat(Math.round((credit-debit)* 100) / 100).toFixed(2);
	            y+=m + " Cr";
	        } else {
	            m=parseFloat(Math.round(((credit-debit)*-1) * 100) / 100).toFixed(2);
	            y+= m + " Dr";
	        }
	        y+='</td><td></td></tr>';
