	...
	$(document).ready(function() {
		...
		<?php
		  ...
		  //  echo $logo;
			...
			$new_content="";
			if(isset($txn)) {
			    $content = "<style> .txt_center { text-align:center; } .txt_right { text-align:right; }  .col_right {text-align: right; padding-right: 10px; } .col_left {text-align: left; width: 30%; } .item_header > th {font-weight: bold; border:0.5px solid; } .item_name > td { border: 0.5px solid; height:20px;} @page {size: A4; size:potrait; } @media print { #original, #duplicate, #triplicate { page-break-before: always; } .main_head { display:table-header-group !important; } .main_body tr:nth-of-type(10n) { page-break-after: always; } .main_foot { display:table-footer-group !important; page-break-after:always !important;}} </style>";
    			for ($j=0; $j < count($typearr); $j++) { 
    				// $content.= '<td>';
    				$new_content.='<table>';
    				$new_content.='<thead>';
    				$new_content.='<tr><th colspan="8"><img src="'.$logo.'" style="width: 85%; margin: 0px auto; height: auto;margin-bottom:2px;padding:2px" alt="Logo" /></th></tr>';
    				$new_content.='<tr><th colspan="2"><b>Invoice No: </b><u>'.$txn[0]->it_txn_no.'</u></th><th colspan="3" style="font-size:1.3em;text-decoration:underline;"><div style="text-align: center">TAX INVOICE - '.$typearr[$j].'</div></th><th colspan="3"><b>Date:</b><u>'.date_format(date_create($txn[0]->it_date), 'd/m/Y').'</u></th>';
                    $new_content.='<tr><th colspan="4">Client Name:<b>'.$txn[0]->ic_name.'</b><br>'.$txn[0]->ic_address.'<br>';
    				if($txn[0]->ic_gst_number !== "") $new_content.= 'GST: '.$txn[0]->ic_gst_number;
    				$new_content.='</th><th colspan="4">Mode: '.$txn[0]->it_mode.'</th></tr>';
    				$new_content.='<tr><th colspan="8">';
    				if(count($txn_transport) > 0 ) $new_content.= 'Transporter: <u>'.$txn_transport[0]->ittd_transporter.'</u> ';
    				if(count($txn_transport) > 0 ) $new_content.= 'L/R No.: <u>'.$txn_transport[0]->ittd_lrno.'</u> ';
    				if(count($txn_transport) > 0 ) $new_content.= 'Date: <u>'.$txn_transport[0]->ittd_date.'</u> ';
    				if(count($txn_transport) > 0 ) $new_content.= 'Transporter GSTIN: <u>'.$txn_transport[0]->ittd_transporter_gstno.'</u> ';
    				if(count($txn_transport) > 0 ) $new_content.= 'Date: <u>'.$txn_transport[0]->ittd_state.'</u>';
    				$new_content.='<br/>';
    				if(count($txn_challans) > 0 ) {
    				    $new_content.= 'Challans: ';
    				    $new_content.=implode(", ", $txn_challans);
    				}
    				if(isset($txn_delivery)) $new_content.= 'Challan No: '.$txn_delivery;
    				$new_content.='</th></tr>';
    				$new_content.='</thead>';
    				
    				$new_content.='<tbody><tr><td colspan="8">';
    				$new_content.= '<table border="0" cellspacing=0 style="width: 100%;border:1px solid; border-radius:5px; display:table;">';
    				$new_content.= '<thead class="main_head">';
    				$new_content.= '<tr class="item_header">';
    				$new_content.= '<th>Sr. No.</th>';
    				$new_content.= '<th>Particulars</th>';
    				$new_content.= '<th>HSN Code</th>';
    				$new_content.= '<th>Unit</th>';
    				$new_content.= '<th>Tax</th>';
    				$new_content.= '<th>Qty</th>';
    				$new_content.= '<th>Rate</th>';
    				$new_content.= '<th>Amount</th>';
    				$new_content.= '</tr>';
    				$new_content.= '</thead>';
    				...
    				if(count($txn_transport) > 0 ) { $line_cnt_total = 12 - $line_cnt; } else { $line_cnt_total = 12 - $line_cnt; }
    				
    				for ($i=0; $i < count($txn_details) ; $i++) { 
    				    $subtot = $subtot + $txn_details[$i]->itp_value;
    				    
    					$tbody_content .= '<tr class="item_name"><td class="txt_center">'.($i+1).'</td><td>';
                    ...
                    for ($i=0; $i < $line_cnt_total; $i++) { 
    				    $tbody_content.= '<tr class="item_name"><td></td><td></td><td></td><td class="detail_center"></td><td class="detail_center"></td><td class="detail_right"></td><td class="detail_right"></td><td class="detail_right"></td></tr>';
    				}
                    for ($i=0; $i < count($taxes); $i++) { 
    					$amt = 0.0;
    					$tmp_flg = false;
    				...
    				$tfoot_content='';
    				$tfoot_content.= '<tfoot class="main_foot">';
