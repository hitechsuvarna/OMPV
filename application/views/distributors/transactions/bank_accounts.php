    ...
    <div class="mdl-grid" style="display:non;">
		<div class="mdl-cell mdl-cell--4-col">
			<div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label">
				<input type="text" data-type="date" id="accounts" class="mdl-textfield__input" value="">
				<label class="mdl-textfield__label" for="accounts">Search Accounts</label>
			</div>
		</div>
		...
		<div class="mdl-cell mdl-cell--12-col">
            ...
            <h4 id="total_amount" style="text-align:right;"></h4>
		</div>
...
<script>
    $(document).ready(function() {
	    $('#update_pay').click(function(e) {
	        ...
	        var url = "";
	        if(edit_flag == true) {
	            url = '<?php echo base_url().$type."/Transactions/update_payment_txn/"; ?>' + $('#accounts').val() + '/' + edit_index;
	        } else {
	            url = '<?php echo base_url().$type."/Transactions/update_payment_txn/"; ?>' + $('#accounts').val() + '/';
	        }
	        $.post(url, {
	            'date' : $('#pay_date').val(),
	            'mode' : $('#pay_mode').val(),
	            ...
	            get_transactions($('#accounts').val(), $('#from_date').val(), $('#to_date').val());
	        })
	    })
        ...
        $('#download').click(function(e) {
		    ...
		    $('.purchase_table').tableExport({
                ...
                filename: $('#accounts').val() + '-' + $('#txn_type').val(),                         
	...
	function get_transactions(accounts, from, to) {
	    $.post('<?php echo base_url().$type."/Transactions/get_payment_bank_txns/"; ?>', {
	        'a' : accounts,
	        'fr' : from,
	        'to' : to
	    }, function(d,s,x) {
	        ...
	        for(var i=0;i<b.length; i++) {
	            ...
	            x+='<tr><td>' + b[i].descr + ' - ' + b[i].cname + '</td><td>' + b[i].date + '</td><td style="text-align:right;">' + b[i].credit + '</td><td style="text-align:right;">' + b[i].debit + '</td><td>';
	            if(b[i].info == "payment") {
	                <?php if($oid == $uid) {
	                    echo 'x+=\'<button class="mdl-button mdl-js--button mdl-button--colored edit_pay" id="\' + b[i].id +\'"><i class="material-icons">create</i></button><button class="mdl-button mdl-js--button mdl-button--colored delete_pay" id="\' + b[i].id + \'"><i class="material-icons">delete</i></button>\';';
	                } ?>
	            } else {
	                x+='<button class="mdl-button mdl-js--button view_txn" id="' + b[i].id + '" tmp="' + b[i].info + '"><i class="material-icons">visibility</i></button><button class="mdl-button mdl-js--button mdl-button--colored select_pay" id="' + b[i].id + '" sel="false" credit="' + b[i].credit + '" debit="' + b[i].debit + '"><i class="material-icons">check_circle_outline</i></button>';
	            }
	            x+='</td></tr>';
	        }
	        a.append(x);
	        $('.purchase_table > tfoot').empty();
	        $('.purchase_table > tfoot').append('<tr><td colspan="2">Total</td><td style="text-align:right;">' + credit + '</td><td style="text-align:right;">' + debit + '</td><td></td></tr>');
	        $('.purchase_table > tfoot').append('<tr><td colspan="3">Balance</td><td style="text-align:right;">' + (credit - debit) + '</td><td></td></tr>');
